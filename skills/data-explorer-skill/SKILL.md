...
**CRITICAL**: 当需求涉及**多个数据源集成**时（如 MongoDB + MySQL），必须先做完整的数据调研，避免绕圈和返工。

```
跨数据源集成清单:
□ 明确每个数据源的角色（主数据源 vs 辅助数据源）
□ 识别关联字段（Join Key）是什么？
□ 验证关联字段在两边是否匹配（字段名、数据类型、实际值）
□ 确认数据重叠度（有多少数据是两边都存在的）
□ 检查数据新鲜度（是否同步、延迟多久）
```

**实战案例：MongoDB + MySQL 集成**

❌ **错误流程**（导致多次返工）:
```
1. 按需求理解开始写代码
2. 用 MongoDB 中存在的 ID 测试
3. 发现 MySQL 中不存在该 ID → 404 错误
4. 才意识到两边数据不匹配
5. 重新调研数据关联关系
```
...
```
第一步：数据源清单
- 主数据源: MySQL nanshan_ent_qxb.nanshan_news_label (权威数据)
- 辅助数据源: MongoDB WaiQian_JingYing_FuChi.new_09_detail (原始 HTML)
...
第三步：数据匹配度验证（核心步骤）
# 脚本：检查两边数据是否有交集
mysql_ids = query_mysql("SELECT DISTINCT md5_url FROM nanshan_news_label LIMIT 100")
mongo_ids = query_mongo({"url_md5": {"$in": mysql_ids}})
match_rate = len(mongo_ids) / len(mysql_ids)

if match_rate < 0.3:
    # 匹配率太低，需要确认数据来源是否一致
    raise DataMismatchError(f"数据匹配率仅 {match_rate:.1%}，请检查数据同步")

第四步：获取可用测试数据
test_ids = [id for id in mysql_ids if id in mongo_ids][:5]
# 用这些 ID 测试，保证两边都有数据
...
**预检脚本模板**:
```python
def validate_cross_datasource_integration(
    primary_db_query: str,
    secondary_db_query: Callable,
    join_key: str,
    sample_size: int = 100
) -> dict:
    """
    ...
    """
    # 1. 从主数据源获取样本
    primary_ids = execute_primary_query(primary_db_query, sample_size)

    # 2. 在辅助数据源中查找匹配
    matched = secondary_db_query({join_key: {"$in": primary_ids}})

    # 3. 计算匹配率
    match_rate = len(matched) / len(primary_ids)

    # 4. 生成警告
    warnings = []
    if match_rate < 0.3:
        warnings.append(f"⚠️ 数据匹配率过低: {match_rate:.1%}")
    if match_rate == 0:
        warnings.append("❌ 无任何匹配数据，请检查字段名和数据同步")

    return {
        'total_primary': len(primary_ids),
        'matched': matched,
        'match_rate': match_rate,
        'test_ids': matched[:5],  # 前5个用于测试
        'warnings': warnings
    }
```
...
- 单一数据源操作（只查询一个库）
- 已验证过的数据源（有文档记录匹配关系）
- LEFT JOIN 场景（辅助数据可为空，不影响主流程）

**预检失败的处理**:
1. 匹配率 = 0 → 停止，检查字段名/数据同步
2. 匹配率 < 30% → 警告，确认数据来源是否一致
3. 匹配率 30-70% → 继续，但标记为部分集成
4. 匹配率 > 70% → 正常，使用匹配的测试数据
